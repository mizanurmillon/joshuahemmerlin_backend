        ...
        if ($userData && Hash::check($request->password, $userData->password)) {
            if ($userData->email_verified_at == null) {
                ...
                if ($request->has('remember')) {
                    $userData->update(['remember_token' => Str::random(10)]);
                }

                if ($userData->role == 'admin') {
                    if ($userData->membership != null && Carbon::now() < $userData->membership->end_date) {
                        $is_membership = true;
                    } else {
                        $is_membership = false;
                    }
                    ...
                    $is_membership = true;
                    $userData->load('userRole.role:id,role_name');
                }

                $userData->setAttribute('token', $token);
                $userData->setAttribute('is_membership', $is_membership);
                $userData->setAttribute('email_verified', $flag);
            }
        } else {
            return $this->error([], 'Invalid credentials', 401);
